...
// interpolation code adapted from http://blog.mackerron.com/2011/01/01/javascript-cubic-splines/
...
let hdiDict = JSON.parse(fs.readFileSync('hdi_dict.json'))

let bidsParsed = JSON.parse(fs.readFileSync('bids_parsed.json'))

let years = [1980, 1985, 1990, 1995, 2000, 2005, 2010, 2011, 2012, 2013, 2014]
let allYears = Array(35).fill().map((_, i) => 1980 + i)

let result = _.mapValues(hdiDict, arr => {

  let xVals = arr.map(entry => entry.year)
  let yVals = arr.map(entry => entry.hdi)

  let mcs = new MonotonicCubicSpline(xVals, yVals)

  let firstYear = arr.find(d => d.hdi !== null).year

  return allYears.filter(y => y >= firstYear)
      .map(x => {
        return {
          'year' : x,
          'hdi' : mcs.interpolate(x)
        }
      })
})


fs.writeFileSync('../interactive/hdi_interpolated.json', JSON.stringify(result, null, 2))

console.log('performed monotonic cubic interpolation on HDI data')